...
def to_binary(jogo):
    """
    Converte dezenas -> vetor binário de 25 posições.
    Se houver valor fora de 1..25, ignora e loga um aviso.
    """
...
def build_dataset_ls14pp(rows, rep_map, window):
    ...
    for i in range(len(rows) - window):
        ...
        # atraso (distância desde última ocorrência de cada dezena)
        ...
        # globais: soma das dezenas e qtd pares
...
def train_ls14pp(last_n=500, window=50, epochs=100, batch=32, lr=1e-3, pos_weight=3.0, out="."):
    ...
    chk_path = os.path.join(out, "modelo_ls14pp.h5")
    cbs = [
        ModelCheckpoint(chk_path, save_best_only=True, monitor="val_loss"),
        EarlyStopping(monitor="val_loss", patience=12, restore_best_weights=True),
        ReduceLROnPlateau(monitor="val_loss", factor=0.5, patience=6),
...
def train(model_name, last_n=None, window=50, epochs=50, batch=32, lr=1e-3, pos_weight=3.0, out="."):
    if model_name.lower() == "ls15":
        ...
        return train_ls14pp(last_n=last_n or 500, window=window, epochs=epochs, batch=batch, lr=lr, pos_weight=pos_weight, out=out)
    else:
        raise ValueError("model_name deve ser 'ls15', 'ls14' ou 'ls14pp'.")

# -------------------- CLI --------------------
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", required=True, choices=["ls15", "ls14", "ls14pp", "both"])
    ...
    if args.model == "ls14pp":
        train_ls14pp(last_n=args.last_n, window=args.window, epochs=args.epochs, batch=args.batch, lr=args.lr,
                     pos_weight=args.pos_weight, out=args.out)
